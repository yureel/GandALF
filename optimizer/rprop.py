...
class Rprop(Minimizer):
    ...
    def __init__(self, wrt, fprime, step_shrink=0.5, step_grow=1.2,
        ...
        self.gradient = zero_like(self.wrt)
        self.changes = zero_like(self.wrt)
    ...
    def _iterate(self):
        for args, kwargs in self.args:
            ...
            self.changes = clip(self.changes, self.min_step, self.max_step)

            step = -self.changes * sign(self.gradient)
            self.wrt += step
